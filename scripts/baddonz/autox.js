(function () {
  ...
  style.textContent = `
  .baddonz-button {
    cursor: url("https://gordion.margonem.pl/img/gui/cursor/5n.png") 4 0, text !important;
  }
  ...
  #baddonz-ax-wnd-settings {
    width: 250px;
    min-width: 250px;
  }

  .baddonz-window-body#ax-wnd-body {
    padding: 0 5px 5px 5px;
    gap: 5px;
  }

  #ax-expanded-controls {
    display: none;
  ...
  #ax-walk-btn, #ax-s-walka-btn {
    width: 100%;
  }

  .baddonz-setting-row.ax-main-row {
    gap: 5px;
  }

  .baddonz-input.ax-small {
    width: 100%;
    max-width: 79px;
    font-size: 11px;
  ...
  #baddonz-ax-wnd-settings .baddonz-input {
    text-align: center;
    height: 26px !important;
  }

  .baddonz-input.ax-hotkey {
    width: 100%;
    padding: 1px 5px;
    font-size: 14px;
    ...
    cursor: url("https://gordion.margonem.pl/img/gui/cursor/5n.png") 4 0, default !important;
  }

  .baddonz-setting-row .baddonz-input {
    flex-grow: 1;
  }
  .baddonz-setting-row span {
    white-space: nowrap;
    font-size: 11px;
  }

  #ax-clan-options, #ax-nick-options {
    display: none;
    flex-direction: column;
    gap: 5px;
    margin-top: 5px;
  ...
  #ax-clan-options-header, #ax-nick-options-header, #ax-hotkey-options-header {
    display: flex;
    align-items: center;
    gap: 5px;
  }
  ...
  .baddonz-textarea::-webkit-scrollbar {
    width: 6px;
    height: 6px;
  }
  .baddonz-textarea::-webkit-scrollbar-track {
    background: #1414148f;
    border-radius: 3px;
  }
  .baddonz-textarea::-webkit-scrollbar-thumb {
    background: #505050;
    border-radius: 3px;
  }
  .baddonz-textarea::-webkit-scrollbar-thumb:hover {
    background: #808080;
  }
  ...
  let fastFightEnabled = false;    // fastFight (toggle)
  let attackClan = false;
  let attackFriends = false;
  let attackWanted = false;
  let walkingOptionEnabled = false;// enableWalkingOption (show walking control section)
  let nickOptionsEnabled = false;
  let hotkeyAttackEnabled = false;
  let onlyHotkeyAttack = false;

  let expandControls = false;      // isExpanded
  let windowOpacityIndex = 2;      // windowOpacity 0..4
  let settingsOpacityIndex = 2;    // windowSettingsOpacity
  ...
  // UI elements refs (created later)
  let autoxWnd = null;
  ...
  // Runtime track last move
  let loopAttackTimer = null;
  let loopBattleTimer = null;

  // Key capture mode for hotkey input
  let waitingForKey = false;
  let waitingInputId = null;

  // Current hero/account IDs for storage scoping
  let heroId = null;
  let accountId = null;
  ...
  function loadSettings() {
    ...
    ignoreNicksList = parseListCommas(acc.ignoreNicks);
  }
  ...
  function saveSettings() {
    ...
    storageData.chars[heroId] = {
      ...
      showAutoxWindow: true, // present in original, always true
      attackFriends,
      attackClan,
      attackWanted,
  ...
  function getEntities() {
    return Engine.map.getDrawableList().map(e => e.d ? e.d : e).filter(Boolean);
  }

  function isEntityInBattleOrProtected(entity) {
    // Check entity's emo list for battle/pvpprotected
    const emoType = Engine.getOnSelfEmoList(entity.id)?.getDrawableList()?.[0]?.name;
    return emoType && emoType !== 'battle' && emoType !== 'pvpprotected';
  }

  // Target validation logic (matches original)
  function isValidTarget(entity) {
    ...
    // Nick filter
    if (nickOptionsEnabled && ignoreNicksList.includes(entity.nick)) {
      return false;
    }

    // Clan handling
    const clanId = entity.clan && typeof entity.clan === 'object' ? entity.clan.id : (entity.clan || null);
    const clanName = entity.clan && typeof entity.clan === 'object' ? entity.clan.name : (entity.clan || null);

    const isAlwaysAttackClan = (cid, name) => {
      return (cid && alwaysAttackClansList.includes(String(cid).toUpperCase())) ||
             (name && alwaysAttackClansList.includes(String(name)));
    };

    const isIgnoredClan = (cid, name) => {
      return (cid && ignoreClansList.includes(String(cid).toUpperCase())) ||
             (name && ignoreClansList.includes(String(name)));
    };

    if (clanOptionsEnabled) {
      if ((clanId || clanName) && isAlwaysAttackClan(clanId, clanName)) return true;
      if ((clanId || clanName) && isIgnoredClan(clanId, clanName)) return false;
    }

    const map = Engine.map?.d;
    // PvP modes (original checks map.pvp, and "others" property used equivalently)
    if (map && (map.pvp === 2 || map.pvp === 4)) {
      // Attack relations 1,3,6
      if ([1, 3, 6].includes(entity.relation)) return true;
    } else if (map && map.pvp === 1) {
      // Attack relation 3, or wanted with attackWanted enabled
  ...
  function filterTargets() {
    ...
    return getEntities()
      ...
      .filter(e => isEntityInBattleOrProtected(e)); // must not be in battle/pvpprotected effect
  }
  ...
  // Fast fight end marker/reset
  let fastFightConsumed = false;
  function markFastFightDoneOnce() {
  ...
  function autoAttackPulse() {
    ...
    // Hotkey-only mode: only act on hotkey press
    if (hotkeyAttackEnabled && onlyHotkeyAttack) {
      // no auto unless user pressed key; the key handler calls single-pulse attack
      return;
    }
    ...
    if (nt) {
      ...
      // No target: if walking back is enabled, go to back coords
  ...
  function consumeFastFightOnceIfAllowed() {
    if (Engine?.battle?.inBattle && !Engine?.battle?.endBattle) {
      if (fastFightEnabled && !fastFightConsumed) {
  ...
  function stopAttackLoop() {
    if (loopAttackTimer) {
      clearInterval(loopAttackTimer);
      loopAttackTimer = null;
    }
  }
  ...
  function stopBattleLoop() {
    if (loopBattleTimer) {
      clearInterval(loopBattleTimer);
      loopBattleTimer = null;
    }
  }

  // Key capture for hotkey input field
  ...
  function onDocumentKeydown(ev) {
    // Hotkey attacks (when not waiting for key input)
    if (!waitingForKey) {
      // ignore if typing into text inputs
    ...
    // We are capturing a key for the input
    ev.preventDefault();
    ev.stopPropagation();
  ...
  function buildUI() {
    ...
    // Apply opacity classes based on indices
    ...
    function renderExpanded() {
      ...
      if (walkingOptionEnabled) {
        ...
        if (walkingEnabled) {
          ...
          backEnabled = false; // disable back if walking disabled
        }
    ...
    // Tooltips (original uses jQuery tip; we set innerText to emulate)
    // ax-collapsed-btn
    // ax-settings-btn
    // ax-walk-btn
    // s.walka button
    ...
    // Show window toggle (exists in original, effectively always true; here we allow hide/show)
    showWindowCheckbox.addEventListener('click', () => {
      ...
      // original stores showAutoxWindow; we keep always true as in source defaults
      saveSettings();
    });
    ...
    // Settings side duplicates (keep in sync)
  ...
  // Hook battle end to reset fast fight "consumed" flag (matches original wrapping of setEndBattle)
  function hookBattleEnd() {
    ...
    if (typeof Engine.battle.setEndBattle === 'function') {
      ...
      Engine.battle.setEndBattle = function () {
        original();
        // When battle ends, allow fast fight again
        fastFightConsumed = false;
      };
    }
  }
  ...
  function initWhenReady() {
    ...
    // Identify hero/account for storage scoping
    heroId = window.Engine.hero.d.id;
    accountId = window.Engine.hero.others.d.account; // original uses Engine.hero.others.d.account
    ...
    // Safety message (original shows tip message)
    // We'll emulate once (optional)
    // message('Używasz na własną odpowiedzialność!');
  }
